...
import { getMcpStats, getScriptStats, getSkillStats } from "../storage/steps-repo.ts";
import { fmtAge, fmtTok, header, keyVal, table, truncateKey } from "./format.ts";
...
export const handleStatus: Handler = (db, _args) => {
  ...
  const stuckLine =
    stuckRows.total === 0 ? "0" : `${stuckRows.total} (${truncateKey(stuckRows.sessions[0]?.session_key || "", 50)})`;
  ...
  const errorBreakdown = errorRows.length > 0 ? errorRows.map((r) => `${r.error_type}:${r.n}`).join(" ") : "none";
...
export const handleStuck: Handler = (_db, _args) => {
  ...
  const rows = result.sessions.map((s) => {
    ...
    return [truncateKey(s.session_key), s.blocker || "—", fmtAge(ageMs), truncateKey(s.current_op || "—", 20)];
  });
  return [
...
export const handleTop: Handler = (db, _args) => {
  ...
  const data = rows.map((r) => [truncateKey(r.session_key), fmtTok(r.total_tokens), r.model || "—"]);
  return [header("observability-v2 top sessions (24h)"), table(["session", "tokens", "model"], data)].join("\n");
};
...
export const handleSkills: Handler = (_db, args) => {
  ...
  const rows = (getSkillStats(range) as any[]).filter((r) => r.call_count > 0).slice(0, 5);
  if (rows.length === 0) {
  ...
  return [header(`observability-v2 skills (${range})`), table(["name", "calls", "avg", "p95", "err"], data)].join("\n");
};
...
export const handleScripts: Handler = (_db, args) => {
  const range = parseRange(args[0]);
  const rows = (getScriptStats(range) as any[]).filter((r) => r.call_count > 0).slice(0, 5);
  if (rows.length === 0) {
  ...
  return [header(`observability-v2 scripts (${range})`), table(["name", "calls", "avg", "p95", "err"], data)].join(
    "\n",
  );
};
...
export const handleMcps: Handler = (_db, args) => {
  ...
  return [header(`observability-v2 mcps (${range})`), table(["name", "calls", "avg", "p95", "err%"], data)].join("\n");
};
...
export const handleErrors: Handler = (db, _args) => {
  ...
  return [header(`observability-v2 errors (latest 10)`), table(["time", "session", "tool", "type", "msg"], data)].join(
    "\n",
  );
};

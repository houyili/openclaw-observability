...
console.log("\n=== Group 1: TypeScript env parser ===");
{
  ...
  assert(
    getEnvValue("OBS_AUTH_TOKEN", envPath, { OBS_AUTH_TOKEN: '"from-process"' }) === "from-process",
    "process env wins and is unquoted",
  );
...
console.log("\n=== Group 2: shell env helper ===");
{
  ...
  const token = execFileSync("sh", ["-c", `. "${script}"; obs_read_env_value OBS_AUTH_TOKEN "${envPath}"`], {
    encoding: "utf-8",
  });
  const domain = execFileSync("sh", ["-c", `. "${script}"; obs_read_env_value OBS_NGROK_DOMAIN "${envPath}"`], {
    encoding: "utf-8",
  });

...
console.log("\n=== Group 1: Schema migration ===");
{
  ...
  for (const w of want) {
    assert(
      cols.some((c) => c.name === w),
      `column ${w} present after migrate()`,
    );
  }
...
const a0Row = getDb()
  .prepare("SELECT input_tokens, cache_read_tokens, thinking_text_len FROM steps WHERE step_id = 'a0'")
  .get() as any;
...
if (breakdown) {
  assert(
    breakdown.totalLatest === 24800,
    "totalLatest == 24800 (last prompt = in + cR = 12500 + 12300)",
    `got ${breakdown.totalLatest}`,
  );
  assert(
    breakdown.frameworkBaseline === 10000,
    "frameworkBaseline == 10000 (in₀ + cR₀ = 10000 + 0)",
    `got ${breakdown.frameworkBaseline}`,
  );
  assert(breakdown.turns.length === 4, "4 turns in breakdown", `got ${breakdown.turns.length}`);
  ...
  assert(
    t2.contributors?.unaccounted === 1920,
  ...
  assert(t3.contributors?.unaccounted === 920, "turn 3 unaccounted == 920", `got ${t3.contributors?.unaccounted}`);

  // Sanity invariant: baseline + Σ Δ == totalLatest
  const sumDelta = breakdown.turns.slice(1).reduce((s, t) => s + (t.deltaFromPrev || 0), 0);
  assert(
...
if (timeline) {
  ...
  assert(tt[1].deltaIn === 11500, "turn 1 deltaIn == 11500 (Δprompt = 21500 - 10000)", `got ${tt[1].deltaIn}`);
  assert(tt[2].deltaIn === 2300, "turn 2 deltaIn == 2300 (Δprompt = 23800 - 21500)", `got ${tt[2].deltaIn}`);
  assert(tt[3].deltaIn === 1000, "turn 3 deltaIn == 1000 (Δprompt = 24800 - 23800)", `got ${tt[3].deltaIn}`);
  ...
  //   TOTAL                510
  assert(
    timeline.cumulative.totalOutputTokens === 510,
  ...
  assert(timeline.cumulative.totalReplyTextChars === 120, "cumulative.totalReplyTextChars == 120");

  // total thinking chars = 11 + 13 + 10 + 5 = 39
  assert(
    timeline.cumulative.totalThinkingChars === 39,
    "cumulative.totalThinkingChars == 39",
    `got ${timeline.cumulative.totalThinkingChars}`,
  );

  // total tool result chars = 1200 + 800 + 0 = 2000
  assert(timeline.cumulative.totalToolResultChars === 2000, "cumulative.totalToolResultChars == 2000");
  ...
  assert(
    timeline.cumulative.cacheHitRate != null && Math.abs(timeline.cumulative.cacheHitRate - expectedHit) < 1e-6,
  ...
  assert(
    timeline.topSpikes[0].seq === tt[1].seq && timeline.topSpikes[0].deltaIn === 11500,
    "top spike #1 is turn 1 with +11500",
  );
  assert(
    timeline.topSpikes[1].seq === tt[2].seq && timeline.topSpikes[1].deltaIn === 2300,
...
console.log("\n=== Group 5: Loop heuristic ===");
{
  ...
  if (loopTimeline) {
    assert(
      loopTimeline.loopFlags.consecutiveNoWriteTurns >= 12,
      "loop: consecutiveNoWriteTurns >= 12",
      `got ${loopTimeline.loopFlags.consecutiveNoWriteTurns}`,
    );
    assert(
      loopTimeline.loopFlags.suspectedLoopWindows.length >= 1,
      "loop: at least one suspectedLoopWindow",
      `got ${loopTimeline.loopFlags.suspectedLoopWindows.length}`,
    );
    assert(loopTimeline.loopFlags.suspectedLoopWindows[0].turns >= 8, "loop: window covers >= 8 turns");
    assert(
      loopTimeline.loopFlags.healthVerdict === "stuck",
  ...
  assert(healthy?.loopFlags.healthVerdict === "healthy", "healthy 4-turn run reports healthy verdict");
}
...
console.log("\n=== Group 6: HTTP route handler ===");
{
  ...
  const sendJson = (_res: any, data: any, s?: number) => {
    payload = data;
    status = s;
  };
  ...
  const sendJson404 = (_res: any, data: any, s?: number) => {
    payload404 = data;
    status404 = s;
  };
  handleSessionsRoutes.context("agent:nonexistent:foo", { runId: "doesnotexist" }, {} as any, sendJson404);
  assert(status404 === 404, "unknown session/run returns 404", `got status ${status404}`);
...
console.log("\n=== Group 7: Cache-aware Δctx ===");
{
  const cacheEntries: any[] = [
    ...
    // Turn 0: cold cache — all 20000 tokens are uncached
    {
      ...
      message: {
        role: "assistant",
        content: [{ type: "toolCall", name: "read", id: "cTC0", arguments: { file_path: "/tmp/a" } }],
    ...
    // Old Δin = 800 - 20000 = −19200 (!!!). Prompt Δ = 20800 - 20000 = 800. ✅
    {
      ...
      message: {
        role: "assistant",
        content: [{ type: "toolCall", name: "read", id: "cTC1", arguments: { file_path: "/tmp/b" } }],
    ...
    // Old Δin = 1200 - 800 = +400. Prompt Δ = 22000 - 20800 = 1200. ✅
    {
      ...
      message: {
        role: "assistant",
        content: [{ type: "toolCall", name: "exec", id: "cTC2", arguments: { command: "ls" } }],
  ...
  if (bd) {
    ...
    assert(bd.frameworkBaseline === 20000, "cache: baseline == 20000", `got ${bd.frameworkBaseline}`);
    assert(bd.totalLatest === 22400, "cache: totalLatest == 22400", `got ${bd.totalLatest}`);
    assert(bd.turns.length === 4, "cache: 4 turns");

    // Δctx for each turn (all positive — no fake negatives!)
    assert(
      bd.turns[1].deltaFromPrev === 800,
      "cache: turn 1 Δctx == 800 (NOT −19200)",
      `got ${bd.turns[1].deltaFromPrev}`,
    );
    assert(bd.turns[2].deltaFromPrev === 1200, "cache: turn 2 Δctx == 1200", `got ${bd.turns[2].deltaFromPrev}`);
    assert(
      bd.turns[3].deltaFromPrev === 400,
    ...
    assert(
      bd.frameworkBaseline + sum === bd.totalLatest,
  ...
  if (tl) {
    ...
    assert(
      tl.cumulative.cacheHitRate != null && Math.abs(tl.cumulative.cacheHitRate - expectRate) < 1e-6,

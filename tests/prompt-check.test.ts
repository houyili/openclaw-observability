  ...
  writeFileSync(
    ...
    `${[
      JSON.stringify({
        ...
        severity: "warning",
        message: "checkpoint before yield",
      }),
      JSON.stringify({
...
console.log("\n=== Group 3: prompt projection rules + hooks ===");
{
  ...
  insertStep({
    ...
    nodeType: "TOOL_CALL",
    toolName: "read",
    input: "/tmp/openclaw-workflow/SKILL.md",
  });
  insertStep({
    id: "source-op",
    seq: 1,
    ...
    input: "paper:token-superposition",
  });
  insertStep({
    id: "spawn",
    seq: 2,
    ...
    result: JSON.stringify({ childSessionKey: childKey, runId: childRun }),
  });
  insertStep({
    id: "checkpoint",
    seq: 3,
    ...
    nodeType: "TOOL_CALL",
    toolName: "write",
    input: "/tmp/source_collection_checkpoint.md",
  });
  ...
  assert(
    check.promptSources.some((s) => s.kind === "rule" && s.exists),
    "prompt rule source is reported",
  );
  assert(check.rules.length >= 5, "demo rule pack evaluated");
  assert(
    check.rules.every((r) => r.status === "ok"),
    "all transcript-backed rules pass in positive fixture",
  );
  assert(check.hooks.length === 2, "hook events attached to session");
  assert(
    check.hooks.some((h) => h.status === "bound"),
    "bound hook event preserved",
  );
  assert(
    check.hooks.some((h) => h.status === "unbound"),
    "missing run/step hook is marked unbound",
  );
  assert(
    check.diagnostics.some((d) => d.type === "unbound_hook_event"),
    "unbound hook diagnostic emitted",
  );
}
...
console.log("\n=== Group 4: prompt projection catches missing evidence ===");
{
  ...
  assert(
    check.rules.some((r) => r.ruleId === "read_source_skill_before_source_ops" && r.status === "warning"),
    "missing source skill read is detected",
  );
  assert(
    check.rules.some((r) => r.ruleId === "checkpoint_before_sessions_yield" && r.status === "warning"),
    "missing checkpoint before yield is detected",
  );
  assert(
    check.rules.some((r) => r.ruleId === "spawn_accept_must_have_child_key_run_id" && r.status === "warning"),
    "missing spawn accepted fields is detected",
  );
}

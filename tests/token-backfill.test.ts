...
function insertSession(key: string, sid: string, opts: {
  totalTokens?: number; inputTokens?: number; outputTokens?: number; contextTokens?: number;
} = {}) {
  ...
  `).run(key, sid,
    ...
    opts.totalTokens ?? 0, opts.contextTokens ?? 0, now);
}

// Helper: insert a step row
let stepSeq = 0;
function insertStep(sessionKey: string, runId: string, opts: {
  nodeType: string; role?: string; toolName?: string;
  totalTokens?: number; outputTokens?: number; inputTokens?: number;
  skillName?: string; mcpTool?: string;
}) {
  const seq = stepSeq++;
  db.prepare(`INSERT INTO steps
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'ok', 0, 0)
  `).run(
    `step-${seq}`, sessionKey, runId, seq, isoNow(-1000 + seq * 100), now - 1000 + seq * 100,
    opts.role ?? 'assistant', opts.nodeType, opts.toolName ?? null,
    opts.totalTokens ?? null, opts.outputTokens ?? null, opts.inputTokens ?? null,
    opts.skillName ?? null, opts.mcpTool ?? null,
  );
}
...

{
  ...
  assert(sess.total_tokens === 1800,
    "total_tokens backfilled from steps (MAX)",
    `got ${sess.total_tokens}`);

  // MAX(input_tokens) = 1740
  assert(sess.input_tokens === 1740,
    "input_tokens backfilled from steps (MAX)",
    `got ${sess.input_tokens}`);

  // SUM(output_tokens) of MODEL_THINK + REPLY = 40 + 80 + 60 + 30 = 210
  assert(sess.output_tokens === 210,
  ...
  assert(sess.context_tokens === 1740,
    "context_tokens backfilled from steps",
  ...
  assert(sess.llm_call_count === 3,
    "llm_call_count = 3 (3 MODEL_THINK)",
    `got ${sess.llm_call_count}`);
  assert(sess.tool_call_count === 2,
    "tool_call_count = 2 (read + write)",
    `got ${sess.tool_call_count}`);
}
...

{
  ...
  assert(sess.total_tokens === 25000,
    "total_tokens preserved from auth-poller (25000)",
    `got ${sess.total_tokens}`);
  assert(sess.input_tokens === 20000,
    "input_tokens preserved from auth-poller (20000)",
    `got ${sess.input_tokens}`);
  assert(sess.output_tokens === 5000,
    "output_tokens preserved from auth-poller (5000)",
    `got ${sess.output_tokens}`);
...

{
  ...
  assert(sess.total_tokens === 3000,
    "NULL total_tokens backfilled to 3000",
    `got ${sess.total_tokens}`);
  assert(sess.input_tokens === 2800,
    "NULL input_tokens backfilled to 2800",
    `got ${sess.input_tokens}`);
  assert(sess.output_tokens === 300,
    "NULL output_tokens backfilled to 300 (200+100)",
    `got ${sess.output_tokens}`);
}
...

{
  ...
  const sessA = db.prepare("SELECT total_tokens, input_tokens, output_tokens FROM sessions WHERE session_key = ?").get(KEY_A) as any;
  const sessB = db.prepare("SELECT total_tokens, input_tokens, output_tokens FROM sessions WHERE session_key = ?").get(KEY_B) as any;

  assert(sessA.total_tokens === 5000,
    "bulk: KEY_A total_tokens backfilled to 5000",
    `got ${sessA.total_tokens}`);
  assert(sessA.output_tokens === 400,
    "bulk: KEY_A output_tokens = 400 (300+100)",
    `got ${sessA.output_tokens}`);
  assert(sessB.total_tokens === 9000,
    "bulk: KEY_B total_tokens backfilled to 9000",
    `got ${sessB.total_tokens}`);
  assert(sessB.output_tokens === 500,
    "bulk: KEY_B output_tokens = 500",
    `got ${sessB.output_tokens}`);

  // Group 2's session should still be untouched
  const sessAuth = db.prepare("SELECT total_tokens FROM sessions WHERE session_key = ?")
    .get("agent:test:subagent:has-auth-tokens") as any;
  assert(sessAuth.total_tokens === 25000,
...

{
  ...
  const sess = db.prepare("SELECT total_tokens, input_tokens, output_tokens FROM sessions WHERE session_key = ?").get(KEY) as any;

  // No MODEL_THINK/REPLY with tokens → stays at 0
  assert(sess.total_tokens === 0,
    "no-usage steps: total_tokens stays 0",
    `got ${sess.total_tokens}`);
  assert((sess.input_tokens || 0) === 0,
    "no-usage steps: input_tokens stays 0",
    `got ${sess.input_tokens}`);
}
...

{
  ...
  insertStep(BASE_KEY, "run-cron", { nodeType: "MODEL_THINK", totalTokens: 7000, outputTokens: 400, inputTokens: 6600 });
  ...
  const sessBase = db.prepare("SELECT total_tokens, output_tokens FROM sessions WHERE session_key = ?").get(BASE_KEY) as any;
  const sessRun = db.prepare("SELECT total_tokens, output_tokens FROM sessions WHERE session_key = ?").get(RUN_KEY) as any;

  assert(sessBase.total_tokens === 7000,
    "cron base key: total_tokens backfilled",
    `got ${sessBase.total_tokens}`);
  assert(sessRun.total_tokens === 7000,
    "cron :run: variant: total_tokens also backfilled via fan-out",
    `got ${sessRun.total_tokens}`);
  assert(sessBase.output_tokens === 550,
    "cron base key: output_tokens = 550 (400+150)",
    `got ${sessBase.output_tokens}`);
  assert(sessRun.output_tokens === 550,
...

{
  ...
  recomputeSessionCounts(KEY);
  const before = db.prepare("SELECT total_tokens, input_tokens, output_tokens FROM sessions WHERE session_key = ?").get(KEY) as any;

  recomputeSessionCounts(KEY);
  const after = db.prepare("SELECT total_tokens, input_tokens, output_tokens FROM sessions WHERE session_key = ?").get(KEY) as any;

  assert(before.total_tokens === after.total_tokens,
...

{
  ...
  const entries = [
    {
      type: "message", id: "u-e2e-1", parentId: "", timestamp: "2026-04-12T14:00:00.000Z",
      message: { role: "user", content: [{ type: "text", text: "hello" }] },
    },
    {
      type: "message", id: "a-e2e-1", parentId: "u-e2e-1", timestamp: "2026-04-12T14:00:02.000Z",
      message: {
        role: "assistant",
        content: [{ type: "thinking" }, { type: "toolCall", name: "read", id: "tc-e2e-1", arguments: { file_path: "/tmp/x" } }],
        usage: { input: 4500, output: 200, cacheRead: 3000, totalTokens: 4700 },
      },
    },
    {
      type: "message", id: "r-e2e-1", parentId: "a-e2e-1", timestamp: "2026-04-12T14:00:04.000Z",
      message: { role: "toolResult", content: [{ type: "text", text: "file content" }] },
    },
    {
      type: "message", id: "a-e2e-2", parentId: "r-e2e-1", timestamp: "2026-04-12T14:00:06.000Z",
      message: {
        role: "assistant",
  ...
  assert(sess.total_tokens === 5350,
    "e2e: total_tokens = 5350 (MAX from steps)",
    `got ${sess.total_tokens}`);
  assert(sess.input_tokens === 5200,
    "e2e: input_tokens = 5200 (MAX input from steps)",
    `got ${sess.input_tokens}`);
  ...
  assert(sess.output_tokens > 0 && sess.output_tokens <= 700,
    `e2e: output_tokens > 0 and reasonable (got ${sess.output_tokens})`,
    `got ${sess.output_tokens}`);
  assert(sess.source === "transcript+auth",
    "e2e: source flipped to transcript+auth",
    `got ${sess.source}`);
  assert(sess.llm_call_count >= 1,
    `e2e: llm_call_count >= 1 (got ${sess.llm_call_count})`);
  assert(sess.tool_call_count >= 1,
    `e2e: tool_call_count >= 1 (got ${sess.tool_call_count})`);
}

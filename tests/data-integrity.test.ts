...
assert(
  ...
  "all node_type values are in the known set",
...
assert(
  ...
  "all status values are in {ok, error, running}",
...
assert(
  ...
  "all role values are in {assistant, toolResult}",
  unknownRoles.length > 0 ? `found: ${JSON.stringify(unknownRoles)}` : "",
);

const distinctErrors = new Set<string>(
  (db.prepare("SELECT DISTINCT error_type FROM steps WHERE error_type IS NOT NULL").all() as any[]).map(
    (r: any) => r.error_type,
  ),
);
...
assert(
  ...
  "all error_type values are in the known set",
...
const errorsWithoutText = (
  db
    .prepare("SELECT COUNT(*) as n FROM steps WHERE status='error' AND (error_text IS NULL OR error_text='')")
    .get() as any
).n;
assert(
  errorsWithoutText === 0,
  "every status='error' row has an error_text",
...
const negDurations = (db.prepare("SELECT COUNT(*) as n FROM steps WHERE duration_ms < 0").get() as any).n;
assert(
  negDurations === 0,
  "no negative duration_ms",
...
assert(
  orphanSkillRefs === 0,
  "every skill_name referenced in steps exists in registry",
...
for (const r of topRuns) {
  ...
  const rows = db
    .prepare("SELECT step_id, seq, ts_epoch_ms, role FROM steps WHERE run_id = ? ORDER BY seq")
    .all(r.run_id) as any[];

  // B1: step_ids unique within this run
  const idSet = new Set(rows.map((x) => x.step_id));
  assert(idSet.size === rows.length, `${tag}: step_ids unique within run`, `${rows.length} rows, ${idSet.size} unique`);
  ...
  let monoTs = true;
  for (let i = 1; i < rows.length; i++) {
    if (rows[i].ts_epoch_ms < rows[i - 1].ts_epoch_ms) {
      monoTs = false;
      break;
    }
  }
  ...
  if (rows.length > 0) {
    assert(rows[0].role === "assistant", `${tag}: first step is role=assistant`, `got role=${rows[0].role}`);
  }
}
...
const sessionBaseKeys = new Set<string>(
  (db.prepare("SELECT DISTINCT session_key FROM sessions").all() as any[]).map((r: any) => toBaseKey(r.session_key)),
);
const stepBaseKeys = new Set<string>(
  (db.prepare("SELECT DISTINCT session_key FROM steps").all() as any[]).map((r: any) => r.session_key),
);
...
const sessionsWithoutStepsPct = sessionBaseKeys.size > 0 ? sessionsWithoutSteps / sessionBaseKeys.size : 0;
// "auth-only session that has never written a transcript yet" is normal;
// allow a fairly generous fraction.
assert(
  sessionsWithoutStepsPct <= 0.5,
...
const taSessionKeys = new Set<string>(
  (db.prepare("SELECT DISTINCT session_key FROM sessions WHERE source = 'transcript+auth'").all() as any[]).map(
    (r: any) => toBaseKey(r.session_key),
  ),
);
...
if (sessionBaseKeys.size > 0) {
  assert(overlap > 0, "at least one session base key has matching steps (watcher is alive)", `overlap=${overlap}`);
}
...
if (!serviceUp) {
  ...
  for (const s of topSkills) {
    const sql = (db.prepare("SELECT COUNT(*) as n FROM steps WHERE skill_name = ?").get(s.name) as any).n;
    assert(sql === s.call_count, `/api/skills "${s.name}" call_count matches SQL`, `api=${s.call_count} sql=${sql}`);
  }
  ...
  assert(apiHealth.steps === stepCount, "/healthz steps count matches SQL", `api=${apiHealth.steps} sql=${stepCount}`);
}
...
console.log("\n=== E. Round 6: Context Length live invariants ===");
{
  ...
  for (const r of sampleRuns) {
    ...
    assert(
      recomputed === breakdown.totalLatest,
    ...
    `)
        .get(r.session_key, r.run_id) as any
    ).p;
    assert(
      timeline.cumulative.peakInputTokens === dbPeak,
...
console.log("\n=== F-stale. current processing/blocker freshness ===");
{
  ...
  const blockersWithoutCurrentStuck = db
    .prepare(`
  ...
  `)
    .all() as any[];
  assert(
    blockersWithoutCurrentStuck.length === 0,
...
console.log("\n=== F. Round 7: Parent-child session integrity ===");
{
  ...
  assert(
    ...
    orphans.length > 0
      ? `${orphans.length} orphans: ${orphans
          .map((o: any) => o.parent_session_key)
          .slice(0, 3)
          .join(", ")}`
      : undefined,
  );
  ...
  const subagentTotal = (
    db.prepare("SELECT COUNT(DISTINCT session_key) as n FROM sessions WHERE channel = 'subagent'").get() as any
  ).n;
  const subagentWithParent = (
    db
      .prepare(
  ...
  assert(
    indexes.some((idx: any) => idx.name === "idx_sessions_parent"),
    "F.4: idx_sessions_parent index exists",
  );
}
...
console.log("\n=== G. Workflow Graph projection self-validation ===");
{
  ...
  for (const c of candidates) {
    ...
    assert(
      graph.validation.checks.length >= 8,
      `${tag}: workflow validation ran core checks`,
      `checks=${graph.validation.checks.length}`,
    );
    assert(graph.runId === c.run_id, `${tag}: graph runId matches requested runId`, `graph=${graph.runId}`);
  }

...
Five test suites enforce correctness automatically:
...
Structural invariants on `parseTranscript`:
...
A single-run grace window (≤ 1 run, ≤ 2 steps) tolerates the race between
the test's clean-parse and the watcher's next tick on an actively-writing
session.

### 4. `cross-check` (`tests/cross-check-official.ts`)
...
### 5. `perf` (`tests/perf-bench.ts`)

Wall-clock + CPU budget for the hot paths. Current measurements on a
21 538-step DB:

```
recomputeAllSessionOps       4.6 ms  (budget 250)
recomputeAllSessionCounts    4.8 ms  (budget 500)
getSkillStats('week')        0.6 ms  (budget 100)
getScriptStats('week')       0.6 ms  (budget 100)
getMcpStats('week')          1.4 ms  (budget 100)
getSummaryStats             17.8 ms  (budget 200)
```
